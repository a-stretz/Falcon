import type { MicState } from '../../hooks/useVoice';

interface Props {
  micState: MicState;
  onStart: () => void;
  onStop: () => void;
  disabled?: boolean;
  timeRemaining?: number | null;
}

export function MicButton({ micState, onStart, onStop, disabled, timeRemaining }: Props) {
  const isListening = micState === 'listening';
  const isProcessing = micState === 'processing';
  const isWarning = isListening && timeRemaining != null && timeRemaining <= 30;

  function formatTime(secs: number): string {
    const m = Math.floor(secs / 60);
    const s = secs % 60;
    return `${m}:${String(s).padStart(2, '0')}`;
  }

  return (
    <div className="flex flex-col items-center gap-2">
      <button
        onClick={isListening ? onStop : onStart}
        disabled={disabled || isProcessing}
        title={isListening ? 'Stop recording' : 'Start speaking'}
        className={`
          relative w-12 h-12 rounded-full flex items-center justify-center
          transition-all duration-200 cursor-pointer
          focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500
          ${isListening
            ? isWarning
              ? 'bg-amber-600 hover:bg-amber-500 shadow-lg shadow-amber-900/40'
              : 'bg-red-600 hover:bg-red-500 shadow-lg shadow-red-900/40'
            : isProcessing
            ? 'bg-zinc-700 cursor-not-allowed'
            : 'bg-zinc-800 hover:bg-zinc-700 border border-zinc-600'
          }
          disabled:opacity-50
        `}
      >
        {/* Pulse ring while listening */}
        {isListening && (
          <span className={`absolute inset-0 rounded-full animate-ping opacity-30 ${isWarning ? 'bg-amber-500' : 'bg-red-500'}`} />
        )}

        {isProcessing ? (
          <svg className="animate-spin h-5 w-5 text-zinc-400" viewBox="0 0 24 24" fill="none">
            <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
            <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z" />
          </svg>
        ) : (
          <svg
            viewBox="0 0 24 24"
            className={`w-5 h-5 ${isListening ? 'text-white' : 'text-zinc-300'}`}
            fill="currentColor"
          >
            <path d="M12 1a4 4 0 0 1 4 4v7a4 4 0 0 1-8 0V5a4 4 0 0 1 4-4z" />
            <path d="M19 10v2a7 7 0 0 1-14 0v-2" stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round" />
            <line x1="12" y1="19" x2="12" y2="23" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
            <line x1="8" y1="23" x2="16" y2="23" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
          </svg>
        )}
      </button>

      {/* Countdown timer */}
      {isListening && timeRemaining != null && (
        <span className={`text-xs font-mono tabular-nums ${isWarning ? 'text-amber-400' : 'text-zinc-500'}`}>
          {formatTime(timeRemaining)}
        </span>
      )}
    </div>
  );
}

interface MicStateIndicatorProps {
  micState: MicState;
  interimTranscript: string;
  timeRemaining?: number | null;
  truncated?: boolean;
}

export function MicStateIndicator({ micState, interimTranscript, timeRemaining, truncated }: MicStateIndicatorProps) {
  if (micState === 'idle' && !truncated) return null;

  return (
    <div className="flex items-start gap-2 p-3 bg-zinc-900 border border-zinc-700 rounded-lg">
      {micState !== 'idle' && (
        <span
          className={`mt-1 w-2 h-2 rounded-full shrink-0 ${
            micState === 'listening' ? 'bg-red-500 animate-pulse' : 'bg-zinc-500 animate-spin'
          }`}
        />
      )}
      <div className="flex-1 min-w-0">
        {micState === 'listening' && (
          <p className="text-xs font-medium text-zinc-500 mb-0.5">
            Listening{timeRemaining != null && timeRemaining <= 30 ? ` — ${timeRemaining}s remaining` : '…'}
          </p>
        )}
        {micState === 'processing' && (
          <p className="text-xs font-medium text-zinc-500 mb-0.5">Processing…</p>
        )}
        {truncated && micState === 'idle' && (
          <p className="text-xs font-medium text-amber-400 mb-0.5">Recording cut off at 3-minute limit — answer may be incomplete</p>
        )}
        {interimTranscript && (
          <p className="text-sm text-zinc-300 leading-relaxed">{interimTranscript}</p>
        )}
      </div>
    </div>
  );
}
